...
- **Stuck Loading Spinner Watchdog**: Detects when mobile WebKit stalls on stale multiplexed HTTP/2 streams and automatically recovers within 6 seconds, eliminating indefinite loading spinners.
...
| **Connection Health** | "Lost connection" banner remains visible even after successful auto-reconnect; mobile WebKit hangs on stale HTTP/2 streams | Automatically dismisses stale disconnect banners upon verified server heartbeat and recovers from stuck loading spinners via client watchdog |
