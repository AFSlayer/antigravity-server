...
- **加载转圈卡死自动恢复**：针对移动端 WebKit 在 HTTP/2 复用流上的挂起现象，若对话加载超过 8 秒，客户端看门狗将自动安全刷新恢复连接。
...
| **连接稳定性与横幅** | 重连成功后“Lost connection”横幅仍持续显示，或移动端 WebKit 在 HTTP/2 复用流上挂起 | 检测到服务器心跳正常后自动清除断联横幅，若加载转圈卡死超过 8 秒则由看门狗自动安全重连 |
