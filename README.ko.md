...
| **iOS 키보드 및 한글 입력** | 하단 Safe Area 여백 잔류, 포커스 시 뷰포트 출렁임, 엔터 시 한글 조합 깨짐("안녕녀" 등) | 상단 내비바 고정, Safe Area 축소, 적응형 듀얼 모드 질문 모달 인터랙션 및 모바일 한글/CJK IME 조합 보호(엔터·백스페이스 시 글자 꼬임 방지) |
| **파일 업로드** | 1MB RPC 텍스트 용량 제한 | 대용량 로그·HAR·데이터셋용 **청크 스트리밍 업로더** |
| **연결 경로** | 구글 서버 중계 | **직접 연결** — 내 도메인, 로컬 네트워크, VPN |
| **서버 재기동 / 연결 단절** | 서버 재시작 시 CSRF 토큰 변경으로 연결 영구 단절, 수동 새로고침 필수 | **새로고침 없는 자동 재연결** — CSRF 토큰 영속화 및 gRPC status 14 변환으로 서버 재시작 시 자동 복구 |
| **메모리 유지보수** | 장기 구동 시 메모리가 수 GB 이상 지속 누적 | **일일 유휴 재시작** — 작업 중단 없이 유휴 시간대에 언어 서버를 자동 재기동하여 메모리를 약 450MB 수준으로 리셋 |
...
- **정밀 가상 키보드 트래킹**: 온스크린 키보드가 올라올 때 상단 내비바를 고정한 채 Safe Area 여백을 0px로 축소하여 대화창이 키보드 상단에 완벽 안착.
- **스크롤 앵커링 및 상단 가드**: 긴 대화창에서 이전 대화를 보기 위해 상단으로 스크롤할 때 연쇄적인 무한 데이터 호출(fetch storm)을 방지하고 스크롤 위치를 안정적으로 고정.
...
- **일일 유휴 재시작 (Daily Idle Restart)**: 최신 버전 상태일 때, 활성 스트림이 0개이고 15분 이상 요청이 없는 유휴(Idle) 시간대에 `language_server`를 안전하게 1회 재기동하여 장시간 누적된 대화 기록 및 언어 서비스 메모리를 자동으로 리셋합니다. 작업 중이거나 활성 트래픽이 감지되면 재시작을 10분씩 지연(defer)하여 작업을 방해하지 않습니다.
- 수동 업데이트 확인 및 실행: `agy-server update`

---

### 🔁 새로고침 없는 자동 재연결 및 세션 유지
언어 서버가 재시작되거나(업데이트 또는 서비스 재시작) 일시적인 네트워크 끊김이 발생할 때:
- **CSRF 토큰 영속화**: 서버 재시작 후에도 동일한 인증 토큰을 유지하여 세션 불일치 차단을 원천 방지합니다.
...
- **로딩 스피너 스턱(Stuck) 자동 탈출 가드**: 모바일 WebKit의 HTTP/2 다중화 스트림 지연으로 인해 네트워크가 유휴 상태인 채 스피너가 30초 이상 지속될 경우 클라이언트 워치독이 즉시 연결을 갱신하여 무한 로딩을 방지합니다.
...
| **가상 키보드 및 스크롤** | iOS Safari 뷰포트 출렁임, 하단 여백 벌어짐 및 긴 대화 상단 스크롤 시 연쇄 무한 호출 발생 | Visual Viewport 오프셋 실시간 추종, Safe Area 0px 축소, 대화창 레이아웃 고정, CSS 스크롤 앵커링 및 상단 스크롤 가드로 연쇄 호출 차단 |
...
| **연결 안정성 및 배너** | 재연결 성공 후에도 'Lost connection' 배너가 계속 노출되거나, 모바일 WebKit에서 HTTP/2 스트림이 지연되는 현상 | 하트비트 정상화 감지 시 배너 자동 제거 및 네트워크 유휴 상태에서 30초 이상 스피너 지속 시 클라이언트 워치독을 통한 자동 복구 |
...
- **CSRF 토큰 영속화 및 정규화**: 소유자 전용 권한(`0600`)으로 안전하게 저장되며, 프록시를 통해 정규화 주입되어 서버 재기동 시에도 세션 거부 없이 안전하게 연결을 유지합니다.
