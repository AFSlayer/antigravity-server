...
- **로딩 스피너 스턱(Stuck) 자동 탈출 가드**: 모바일 WebKit의 HTTP/2 다중화 스트림 지연으로 인해 스피너가 8초 이상 지속될 경우 클라이언트 워치독이 즉시 연결을 갱신하여 무한 로딩을 방지합니다.
...
| **연결 안정성 및 배너** | 재연결 성공 후에도 'Lost connection' 배너가 계속 노출되거나, 모바일 WebKit에서 HTTP/2 스트림이 지연되는 현상 | 하트비트 정상화 감지 시 배너 자동 제거 및 8초 이상 스피너 지속 시 클라이언트 워치독을 통한 자동 복구 |
