...
| **입력 방식** | 모바일 엔터 시 즉시 전송 또는 IME 깨짐; 슬래시 커맨드/칩 존재 시 줄 시작 이동 오작동 | 네이티브 줄바꿈 유지, 엔터/백스페이스 시 한글 IME 조합 보존; 슬래시 커맨드/태그 포함 시 Cmd/Ctrl+Left 줄 시작 정상 이동; Cmd/Ctrl+Enter로 전송 |
