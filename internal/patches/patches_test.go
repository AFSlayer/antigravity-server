...
var regexpFixtures = map[string]string{
	...
	"mobile-conversation-gradient-remove":       `style:{background:c==="sidebar"?` + "`" + `linear-gradient(to right, transparent 0%, ${O?"var(--sidebar-secondary)":ia?"color-mix(in srgb, var(--sidebar-secondary) 50%, var(--sidebar))":"var(--sidebar)"} 25%)` + "`" + `:"linear-gradient(to right, transparent 0%, var(--muted) 25%)"}`,
...
func TestPatchedContentIsCorrect(t *testing.T) {
	...
	want := []string{
		...
		`className:Pm("absolute top-0 bottom-0 -right-1 pl-6 flex items-center justify-end gap-0.5 z-10",w||ua?"opacity-100":"opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 focus-within:opacity-100")`,
		`style:{background:"transparent"}`,
		`handleDelete:agyDel,showDeleteModal:agyShowDel`,
		`L.push({iconName:"delete",tooltip:"Delete",onClick:()=>{agySetShowDel(!0)}})`,
		`G.createElement(hlb,{isOpen:agyShowDel,onClose:agyCloseDel,onDelete:agyDel,showLoadingSpinner:agyDelSpin})`,
